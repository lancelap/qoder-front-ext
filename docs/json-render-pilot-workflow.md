...
The goal is to prove that an analyst spec, component catalog, generated `screen.render.json`, and project `registry.tsx` can describe one real screen without inventing behavior.
...
- schema version;
- screen metadata;
- data source ids and API references;
...
Generation is blocked when required components, API contracts, actions, states, or permissions are missing.
...
7. Create or update `component-catalog.md`.
8. Validate catalog coverage with `json-render/00-validate-catalog-coverage.md`.
9. Generate `screen.render.json` with `json-render/01-generate-json-render-spec.md`.
10. Review generated JSON with `json-render/02-review-generated-spec.md`.
11. Draft tests from Given/When/Then with `json-render/03-generate-tests-from-gwt.md`.
12. Only after the pilot JSON passes review, connect `@json-render/react` in a project branch.
...
- Every action and data source in JSON is traceable to `spec.md`.
